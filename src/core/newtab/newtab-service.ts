...
export class NewTabService implements StyleService {
    ...
    enable(): void {
        ...
        if (!this.listenersRegistered) {
            ...
            if (workspace) {
                ...
                this.plugin.registerEvent(workspace.on("active-leaf-change", () => this.apply()));
                ...
                this.plugin.registerEvent(workspace.on("resize", () => this.apply()));
    ...
    private readCurrentAccent(): string {
        ...
        if (!doc?.body) return "";
        return (
            doc.defaultView?.getComputedStyle(doc.body).getPropertyValue("--color-accent") ?? ""
        ).trim();
    }
        ...
        for (const doc of getAppDocuments(this.app)) {
            ...
            for (const vc of contents) {
                ...
                const hasRealContent =
                    ownChildCount > 0 && !(ownChildCount === 1 && vc.querySelector(".empty-state"));
                if (!hasRealContent) {
                    targets.add(vc);
                }
            }
    ...
    private updateInstance(instance: NewTabInstance, s: NewTabSettingsView): void {
        ...
        const engineMissing =
            s.newTabParticleEnabled && (!instance.engine || instance.engine.isDestroyed());
    ...
    private resolveThemeAccent(container: HTMLElement): string {
        try {
            ...
            for (const variable of ["--color-accent", "--interactive-accent", "--text-accent"]) {
    ...
    private applyLogo(instance: NewTabInstance, s: NewTabSettingsView): void {
        ...
        if (s.newTabLogoType === "default") {
            ...
            const svg = this.parseSvg(this.uniqueifySvg(DEFAULT_LOGO_SVG, instance.svgUid));
            if (svg) {
        ...
        if (s.newTabLogoType === "icon") {
            ...
            if (icon) {
                ...
                if (s.newTabLogoBuiltinCustomColor) {
                    const color = resolveAccentValue(
                        s.newTabLogoBuiltinColor,
                        "var(--color-accent)",
                    );
        ...
        if (s.newTabLogoType === "image") {
            ...
            if (folder && s.newTabLogoImageIndex >= 0) {
                ...
                if (images.length > 0) {
                    const idx =
                        ((s.newTabLogoImageIndex % images.length) + images.length) % images.length;
