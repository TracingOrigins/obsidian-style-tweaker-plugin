...
export class ParticleEngine {
    ...
    private readonly handleMouseMove = (event: MouseEvent): void => {
        ...
        this.mouse.x = event.clientX - rect.left + (this.cssWidth - rect.width) / 2;
        this.mouse.y = event.clientY - rect.top + (this.cssHeight - rect.height) / 2;
    };
    ...
    private readonly handleResize = (): void => {
        ...
        this.resizeTimer = this.docWin.setTimeout(() => {
            ...
            if (
                ...
                return;
            const now = Date.now();
            this.rebuildTimestamps = this.rebuildTimestamps.filter((time) => now - time < 5000);
    ...
    private async resample(): Promise<void> {
        ...
        try {
            ...
            if (this.destroyed || token !== this.buildToken || !this.canvas || !this.renderContext)
                return;
    ...
    private async waitForContentSize(token: number): Promise<number> {
        ...
        while (!this.destroyed && token === this.buildToken) {
            ...
            if (this.docWin.performance?.now?.() ?? Date.now() - started >= BUILD_LAYOUT_WAIT_MS)
                break;
        }
    ...
    private collectSources(
        ...
        const logoContainer = this.container.querySelector<HTMLElement>(
            ".style-tweaker-new-tab-logo",
        );
        if (logoContainer) {
        ...
        if (heading && heading.textContent && heading.textContent.trim().length > 0) {
            ...
            if (rect.width > 0 && rect.height > 0) {
                ...
                const style = (heading.ownerDocument.defaultView ?? window).getComputedStyle(
                    heading,
                );
                ...
                if (measureContext && style.fontSize) {
                    ...
                    if (style.textAlign === "center") textX += (rect.width - metrics.width) / 2;
                        ...
                        textX += rect.width - metrics.width;
                    region = {
    ...
    private waitForImage(image: HTMLImageElement): Promise<void> {
        ...
        return new Promise((resolve, reject) => {
            ...
            image.addEventListener("error", () => reject(new Error("Logo image failed to load")), {
                once: true,
            });
        });
    }
    ...
    private sampleParticles(offscreen: HTMLCanvasElement, ops: DrawOp[]): Particle[] {
    ...
    private renderHeartbeat(
        ...
        for (let i = 0; i < particles.length; i++) {
            ...
            const stretch =
                heartbeatShape(time * HEARTBEAT_SPEED - distance * HEARTBEAT_NUMBER) * envelope;
    ...
    private renderRipple(
        ...
        for (let i = 0; i < particles.length; i++) {
            ...
            const distance = Math.sqrt(dx * dx + dy * dy);
            const offset =
                lutSin(time * RIPPLE_SPEED - distance * RIPPLE_NUMBER) * RIPPLE_AMPLITUDE;
            if (distance > 0.001) {
