...
//   - 采用 JS 逐窗口注入 <style>，与 BackgroundService 同构：覆盖独立设置
//     窗口等 pop-out 场景。
//   - CSS 规则本体放在静态 styles.css（src/styles/editor/editor-bg.css），以门控类
//     .style-tweaker-editor-bg 为前缀；本服务只注入 token 变量（统一颜色/间距/附着）
//     并挂摘门控类。
//   - 图案透明度按深浅色在 CSS 中写死（网格 浅25%/深12%、点阵 浅50%/深12%），
//     无需 JS 感知主题，因此不监听主题切换。
// ============================================================

const STYLE_ID = "style-tweaker-editor-bg";

// 门控类：仅在编辑器背景开启（type !== "none"）时挂到各窗口文档，
// 静态 editor-bg.css 里所有图案规则以该类为前缀，关闭时移除类即整体失效。
...
// 编辑器背景统一使用的 CSS 变量名（值由 JS 在运行时注入到 :root）。
...
export class EditorBackgroundService extends BaseService {
  ...
  protected applyToDocument(doc: Document): void {
    ...
    // 注入 token 变量 + 图案 CSS（静态 editor-bg.css 规则本体由 styles.css 加载，
    // 但以门控类为前缀，需配合下方挂摘的类才可见）。
  ...
  private buildTokensCss(s: StyleTweakerSettings): string {
    ...
    const color = this.resolveColor(s.editorBgColor);
  ...
  /** 色名映射为 hex；default/空值回退默认图案灰 #c7c7c7 */
  private resolveColor(value: string | undefined): string {
    return resolveAccentValue(value, "#c7c7c7");
  }
