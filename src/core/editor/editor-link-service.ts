...
// 变量（--style-tweaker-link-internal / --style-tweaker-link-external）；
// 仅低频事件驱动（onLayoutReady / layout-change / window-open），无轮询。
// ============================================================

const STYLE_ID = "style-tweaker-link";
...
export class EditorLinkService extends BaseService {
  ...
  private buildTokensCss(s: StyleTweakerSettings): string {
    ...
    return [
      resolveAccentCss(s.linkInternalColor, LINK_INTERNAL_VAR, "var(--link-color)"),
      resolveAccentCss(s.linkExternalColor, LINK_EXTERNAL_VAR, "var(--link-external-color)"),
    ].join("\n");
  }
