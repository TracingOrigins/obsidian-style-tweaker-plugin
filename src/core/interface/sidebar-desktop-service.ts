...
const TRADITIONAL_ICON_CSS = css`
    ...
    body:not(.is-mobile).theme-light
        .style-tweaker-legacy-profile
        .workspace-drawer-vault-switcher-icon::before,
    body:not(.is-mobile).theme-light
        .style-tweaker-legacy-profile
        .workspace-drawer-vault-switcher-icon:hover::before,
...
export class SidebarDesktopService {
    ...
    enable(): void {
        ...
        const profile = document.querySelector<HTMLElement>(".workspace-sidedock-vault-profile");
    ...
    private findLeftRibbon(): HTMLElement | null {
        const candidates = [".workspace-ribbon.side-dock-ribbon", ".workspace-ribbon"];
 ...
 */
const INTERFACE_FONT_STACK = "var(--font-interface, var(--font-interface-theme, sans-serif))";
...
export class SidebarVaultNameService extends BaseService {
    ...
    protected applyToDocument(doc: Document): void {
        ...
        doc.body.style.setProperty(
            VAULT_NAME_FONT_VAR,
            resolveVaultNameFontFamily(s.vaultNameFontInFileList, s.vaultNameCustomFontInFileList),
        );
