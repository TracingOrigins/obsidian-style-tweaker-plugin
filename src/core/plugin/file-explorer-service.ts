...
const REMOVE_FIRST_FOLDER_ICON_CLASS = "style-tweaker-fe-remove-first-folder-icon";
...
export class FileExplorerService {
    ...
    private applyToDocument(doc: Document): void {
        ...
        const palette = isDark ? s.feColorfulFolderPaletteDark : s.feColorfulFolderPaletteLight;
        ...
        for (let i = body.classList.length - 1; i >= 0; i--) {
            ...
            if (cls.startsWith(COLORFUL_MODE_PREFIX) || cls.startsWith(COLORFUL_PALETTE_PREFIX)) {
    ...
    private updateFolderCounts(doc: Document): void {
        ...
        doc.querySelectorAll('[data-type="file-explorer"] .nav-folder-title').forEach((el) => {
    ...
    private clearFolderCounts(doc: Document): void {
        doc.querySelectorAll('[data-type="file-explorer"] .nav-folder-title[data-count]').forEach(
            (el) => el.removeAttribute("data-count"),
        );
    }
    ...
    private syncFolderCountObserver(doc: Document, on: boolean): void {
        ...
        if (existing) return;
        const container = doc.querySelector('[data-type="file-explorer"] .nav-files-container');
        if (!container) return;
    ...
    private syncCollapseIconClass(doc: Document, on: boolean): void {
        ...
        if (on) {
            ...
            doc.querySelectorAll("." + COLORFUL_COLLAPSE_ICON_CLASS).forEach((svg) =>
                svg.classList.remove(COLORFUL_COLLAPSE_ICON_CLASS),
            );
            if (existing) {
                existing.disconnect();
    ...
    private clearAll(): void {
        for (const doc of new Set([...this.touchedDocuments, ...getAppDocuments(this.app)])) {
            ...
            bodyEl?.classList.remove(FOLDER_MARKER_DOT_CLASS, FOLDER_MARKER_COUNT_CLASS);
            // 清理文件名换行 / 悬停显示后缀标签相关 class
            bodyEl?.classList.remove(FILE_NAME_WRAP_CLASS, HOVER_REVEAL_FILE_TAG_CLASS);
            doc.querySelectorAll(
