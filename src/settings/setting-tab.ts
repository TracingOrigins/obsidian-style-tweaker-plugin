...
export class StyleTweakerSettingTab extends PluginSettingTab {
    ...
    setControlValue(key: string, value: unknown): void | Promise<void> {
        ...
        if (
            ...
            /^(?:desktop|mobile)NewTab(?:LogoType|ParticleEnabled|ParticleCustomColor|TitleType)$/.test(
                key,
            )
        ) {
            this.update();
        ...
        switch (ctrl.type) {
            ...
            case "dropdown": {
                setting.addDropdown((d) => {
                    ...
                    d.setValue(String(settings[key])).onChange((v) => this.setControlValue(key, v));
            ...
            case "folder": {
                ...
                setting.addText((t) => {
                    comp = t as unknown as { setValue: (v: unknown) => void };
                    t.setPlaceholder("Vault path, e.g. Images/wallpapers")
            ...
            case "file": {
                ...
                const preview = setting.controlEl.createDiv({
                    cls: "style-tweaker-image-preview" + (isMobileField ? " is-mobile-field" : ""),
                });
            ...
            case "logo-code": {
                ...
                const updatePreview = (value: string): void => {
                    ...
                    if (trimmed.includes("<svg")) {
                        try {
                            let svg: Element | null = new DOMParser()
            ...
            case "logo-image": {
                ...
                const preview = setting.controlEl.createDiv({
                    cls: "style-tweaker-image-preview is-square" + (isCompact ? " is-compact" : ""),
                });
                const thumb = preview.createEl("img");
                const updateThumb = (path: string) => {
                    if (path) {
                ...
                comp = {
                    setValue: (v: unknown) => {
                        ...
                        settings[key] = n;
                        updateThumb(
                            n >= 0 && images0.length > 0
                                ? images0[normalize(n, images0.length)].path
                                : "",
                        );
                    },
                };
